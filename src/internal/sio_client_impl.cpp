...
namespace sio
{
        ...
        m_reconn_made(0)
    {
        ...
        m_client.set_open_handler([this]<typename T0>(T0&& PH1) { on_open(std::forward<T0>(PH1)); });
        m_client.set_close_handler([this]<typename T0>(T0&& PH1) { on_close(std::forward<T0>(PH1)); });
        m_client.set_fail_handler([this]<typename T0>(T0&& PH1) { on_fail(std::forward<T0>(PH1)); });
        m_client.set_message_handler([this]<typename T0, typename T1>(T0&& PH1, T1&& PH2)
        {
            on_message(std::forward<T0>(PH1), std::forward<T1>(PH2));
        });
#if SIO_TLS
        m_client.set_tls_init_handler(std::bind(&client_impl::on_tls_init,this,_1));
#endif
        m_packet_mgr.set_decode_callback([this]<typename T0>(T0&& PH1) { on_decode(std::forward<T0>(PH1)); });

        m_packet_mgr.set_encode_callback([this]<typename T0, typename T1>(T0&& PH1, T1&& PH2)
        {
            on_encode(std::forward<T0>(PH1), std::forward<T1>(PH2));
        });
    }
    
    client_impl::~client_impl()
    {
        this->sockets_invoke_void(&socket::on_close);
        sync_close();
    }
    ...
    void client_impl::connect(const string& uri, const map<string,string>& query, const map<string, string>& headers, const message::ptr& auth)
    {
        ...
        m_client.get_io_service().dispatch([this, uri] { connect_impl(uri, m_query_string); });
        m_network_thread.reset(new thread([this] { run_loop(); }));//uri lifecycle?

    }

    socket::ptr const& client_impl::socket(string const& nsp)
    {
        lock_guard guard(m_socket_mutex);
        string aux;
        if(nsp.empty())
        {
            aux = "/";
        ...
        const auto it = m_sockets.find(aux);
        if(it!= m_sockets.end())
        {
            return it->second;
        }
    ...
    void client_impl::close()
    {
        ...
        this->sockets_invoke_void(&socket::close);
        m_client.get_io_service().dispatch([this] { close_impl(close::status::normal, "End by user"); });
    }
    ...
    {
        m_client.clear_access_channels(log::alevel::all);
        m_client.set_access_channels(log::alevel::connect | log::alevel::disconnect | log::alevel::app);
    }
    ...
    {
        m_client.set_access_channels(log::alevel::all);
    }

    /*************************protected:*************************/
    void client_impl::send(packet& p) const
    {
    ...
    {
        lock_guard guard(m_socket_mutex);
        const auto it = m_sockets.find(nsp);
        if(it!= m_sockets.end())
        {
            m_sockets.erase(it);
        }
    ...
    void client_impl::on_socket_closed(string const& nsp) const
    {
        if(m_socket_close_listener)m_socket_close_listener(nsp);
    }

    void client_impl::on_socket_opened(string const& nsp) const
    {
    ...
    void client_impl::run_loop()
    {
        ...
        m_client.run();
        m_client.reset();
        m_client.get_alog().write(log::alevel::devel,
                                  "run loop end");
    }
    ...
    void client_impl::connect_impl(const string& uri, const string& queryString)
    {
        do{
            ...
            error_code ec;
            const client_type::connection_ptr con = m_client.get_connection(ss.str(), ec);
            if (ec) {
                m_client.get_alog().write(log::alevel::app,
    ...
    void client_impl::timeout_ping(const lib::asio::error_code &ec)
    {
        if(ec)
        {
            return;
        ...
        m_client.get_io_service().dispatch([this] { close_impl(close::status::policy_violation, "Ping timeout"); });
    }

    void client_impl::timeout_reconnect(lib::asio::error_code const& ec)
    {
        if(ec)
        {
            return;
        ...
        if(m_con_state == con_closed)
        {
            ...
            m_client.get_io_service().dispatch([this] { connect_impl(m_base_url, m_query_string); });
        }
    }
    ...
    unsigned client_impl::next_delay() const
    {
        ...
        const unsigned reconn_made = min<unsigned>(m_reconn_made,32);//protect the pow result to be too big.
    ...
    void client_impl::sockets_invoke_void(void (socket::*fn)(void))
    {
    ...
    void client_impl::on_open(const connection_hdl& con)
    {
        if (m_con_state == con_closing) {
        ...
        m_con = con;
        m_reconn_made = 0;
        this->sockets_invoke_void(&socket::on_open);
        this->socket("");
        if(m_open_listener)m_open_listener();
    }
    
    void client_impl::on_close(const connection_hdl& con)
    {
        LOG("Client Disconnected." << endl);
        const con_state m_con_state_was = m_con_state;
        m_con_state = con_closed;
        error_code ec;
        close::status::value code = close::status::normal;
        const client_type::connection_ptr conn_ptr  = m_client.get_con_from_hdl(con, ec);
        if (ec) {
        ...
        else
        {
            ...
            if(m_reconn_made<m_reconn_attempts && !m_abort_retries)
            {
                ...
                lib::asio::error_code ec;
                m_reconn_timer->expires_from_now(milliseconds(delay), ec);
                m_reconn_timer->async_wait([this]<typename T0>(T0&& PH1) { timeout_reconnect(std::forward<T0>(PH1)); });
                return;
            }
    ...
    void client_impl::on_message(connection_hdl, const client_type::message_ptr& msg)
    {
    ...
    void client_impl::on_handshake(message::ptr const& message)
    {
        if(message && message->get_flag() == message::flag_object)
        {
            ...
            if (it!= values->end()&&it->second->get_flag() == message::flag_integer) {
                m_ping_interval = static_cast<unsigned>(static_pointer_cast<int_message>(it->second)->get_int());
            }
            else
            {
            ...
            if (it!=values->end()&&it->second->get_flag() == message::flag_integer) {
                m_ping_timeout = static_cast<unsigned>(static_pointer_cast<int_message>(it->second)->get_int());
            }
            else
            {
...
failed:
        //just close it.
        m_client.get_io_service().dispatch([this] { close_impl(close::status::policy_violation, "Handshake error"); });
    }
    ...
    void client_impl::on_ping()
    {
        ...
        m_packet_mgr.encode(p, [&](bool /*isBin*/, const shared_ptr<const string>& payload)
        {
    ...
    void client_impl::on_decode(packet const& p)
    {
        ...
        {
            const socket::ptr so_ptr = get_socket_locked(p.get_nsp());
    ...
    void client_impl::on_encode(bool isBinary,shared_ptr<const string> const& payload)
    {
        ...
        m_client.get_io_service().dispatch(
            [this, payload, capture0 = isBinary ? frame::opcode::binary : frame::opcode::text]
            {
                send_impl(payload, capture0);
            });
    }
    ...
    void client_impl::update_ping_timeout_timer() {
        if (!m_ping_timeout_timer) {
            m_ping_timeout_timer = std::make_unique<lib::asio::steady_timer>(get_io_service());
        }

        lib::asio::error_code ec;
        m_ping_timeout_timer->expires_from_now(milliseconds(m_ping_interval + m_ping_timeout), ec);
        m_ping_timeout_timer->async_wait([this]<typename T0>(T0&& PH1) { timeout_ping(std::forward<T0>(PH1)); });
    }
    ...
    std::string client_impl::encode_query_string(const std::string &query){
        ...
        for(const char c : query){
            if((c >= 'a' && c <= 'z') || (c>= 'A' && c<= 'Z') || (c >= '0' && c<= '9')){
                ...
                ss << '%' << std::uppercase << std::setw(2) << static_cast<int>(static_cast<unsigned char>(c)) << std::nouppercase;
            }
        }
        ss << std::dec;
        return ss.str();
